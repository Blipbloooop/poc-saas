...
import { Plus, Search, MapPin, FileText, ShoppingCart, HardHat, ChevronRight, X } from "lucide-react";
...
// ─── Dialog choix du type de création ────────────────────────────────────────

type CreationMode = "devis" | "bon_commande";

function CreationTypeDialog({
  onSelect,
  onClose,
}: { onSelect: (mode: CreationMode) => void; onClose: () => void }) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 backdrop-blur-[2px]" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h2 className="text-base font-semibold text-slate-900">Créer un chantier</h2>
            <p className="text-xs text-slate-400 mt-0.5">Choisissez comment ce chantier démarre</p>
          </div>
        ...
        <div className="p-6 grid grid-cols-2 gap-4">
          {[
            ...
            >
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center transition-colors" style={{ backgroundColor: primaryColor + "15", color: primaryColor }}>
                {icon}
              </div>
              <div>
              ...
              <div className="flex items-center gap-1 text-xs font-medium mt-1" style={{ color: primaryColor }}>
