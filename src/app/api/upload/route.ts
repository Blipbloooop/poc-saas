...
export async function POST(req: NextRequest) {
  ...
  if (kind === "logo") {
    ...
    const blob = await put(`logos/${organizationId}/${safeName}`, file, { access: "public" });
    return NextResponse.json({ success: true, url: blob.url, nom: file.name, taille: file.size, mimeType: file.type });
  }
  ...
  const blob = await put(`documents/${subDir}/${safeName}`, file, { access: "public" });
